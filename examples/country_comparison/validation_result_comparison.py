...
validation_results_country1 = pd.read_csv(
    f"../../data/validation/lpg_example/{country1}/validation_results/default/indicators_per_category.csv",
    index_col=[0, 1],
)
validation_results_country2 = pd.read_csv(
    f"../../data/validation/lpg_example/{country2}/validation_results/default/indicators_per_category.csv",
    index_col=[0, 1],
)
...
absolute_diff_profile = (
    validation_results_country1.groupby(level=0).mean()
    - validation_results_country2.groupby(level=0).mean()
)
relative_diff_profile = (
    absolute_diff_profile / validation_results_country2.groupby(level=0).mean()
)
...
absolute_diff_activity = (
    validation_results_country1.groupby(level=1).mean()
    - validation_results_country2.groupby(level=1).mean()
)
relative_diff_activity = (
    absolute_diff_activity / validation_results_country2.groupby(level=1).mean()
)
...
for i, metric in enumerate(metrics):
    df = (
        relative_diff[metric]
        .reset_index(level=1)
        .pivot(columns="level_1", values=metric)
    )
    sns.heatmap(
        df,
        ax=axs_flat[i],
        ...
        cmap="RdYlGn_r",
        center=0.0,
        vmin=relative_diff[metric].quantile(0.05),
        vmax=relative_diff[metric].quantile(0.95),
    )
