...
if hetus_constants.get_resolution(country1) != hetus_constants.get_resolution(country2):
    for _, v in (
        validation_dataset_country1.statistics | validation_dataset_country2.statistics
    ).items():
        ...
        v.probability_profiles.columns = [
            i * current_resolution
            for i in range(0, len(v.probability_profiles.columns))
        ]
        v.probability_profiles = (
            v.probability_profiles.T.groupby(lambda x: x // 30 * 30).mean().T
        )
        v.probability_profiles.columns = [
            f"MACT{i}" for i in range(1, len(v.probability_profiles.columns) + 1)
        ]
...
activities = (
    ...
    if len(activities) > 0
        ...
        set(validation_dataset_country1.activity_durations.columns)
        | set(validation_dataset_country2.activity_durations.columns)
    )
)
width = 16 * CM_TO_INCH
ncols = 2
...
for i in range(len(activities)):
    axs.append(
        [
            fig.add_subplot(gs[i * 2, 0]),
            fig.add_subplot(gs[i * 2, 1]),
            fig.add_subplot(gs[(i * 2) + 1, :]),
        ]
    )
plt.subplots_adjust(
    left=0.12, right=0.99, top=0.93, bottom=0.1, hspace=0.6, wspace=0.25
)
...
for i, activity in enumerate(activities):
    ...
    merged_df_durations = pd.merge(
        left=validation_dataset_country1.statistics[
            ProfileCategory.from_iterable(
                [country1, profile[0], profile[1], profile[2]]
            )
        ].activity_durations[activity],
        right=validation_dataset_country2.statistics[
            ProfileCategory.from_iterable(
                [country2, profile[0], profile[1], profile[2]]
            )
        ].activity_durations[activity],
        left_index=True,
        right_index=True,
    ...
    for df in [merged_df_durations, merged_df_frequencies]:
        ...
        df.dropna(
            subset=[f"{activity}_{country1}", f"{activity}_{country2}"],
            how="all",
            inplace=True,
        )
        ...
        df.rename(
            columns={
                f"{activity}_{country1}": country1,
                f"{activity}_{country2}": country2,
            },
            inplace=True,
        )
    ...
    profile_country1 = validation_dataset_country1.statistics[
        ProfileCategory.from_iterable([country1, profile[0], profile[1], profile[2]])
    ].probability_profiles.loc[activity, :]
    profile_country2 = validation_dataset_country2.statistics[
        ProfileCategory.from_iterable([country2, profile[0], profile[1], profile[2]])
    ].probability_profiles.loc[activity, :]
    ...
    melted_df_profile = merged_df_profile.melt(
        id_vars="id", var_name="Category", value_name="Value"
    )
